...
export const getPresignedUrlApi = (fileData) => {
  return api.post("/api/users/presigned-url", fileData);
};

// 4. 프로필 이미지 저장/업데이트 API
export const updateProfileImageApi = (imageData) => {
  return api.patch("/api/users/profile-image", imageData);
};

// 5. 비밀번호 변경 API
export const changePasswordApi = (passwordData) => {
  return api.post("/api/users/change-password", passwordData);
};

// 6. 이메일 변경 API
export const changeEmailApi = (emailData) => {
  return api.post("/api/users/change-email", emailData);
};
